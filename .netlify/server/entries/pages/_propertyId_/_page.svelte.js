...
const Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `<div class="${""}"><div class="${"flex gap-5 p-5 rounded-lg border border-[#d7d7d7]"}"><div class="${"flex-1 "}"><div class="${"w-96 h-96 rounded-md"}"${add_attribute(
    ...
    0
        ...
        <div class="${"flex flex-[2] w-full"}"><div class="${"flex-[3]"}"><h1 class="${"text-2xl font-semibold"}">Joyville Western Heights</h1>
                <div class="${"grid grid-cols-3 mt-2 gap-y-4"}"><div><h1 class="${"text-[#606060]"}">Configuration</h1>
                    ...
                    <div><h1 class="${"text-[#606060]"}">Status</h1>
                        <h2>Ready to move</h2></div></div>
                <p class="${"mt-2 text-[#606060]"}">Lorem ipsum, dolor sit amet consectetur adipisicing elit.
            ...
            <div class="${"flex-1 p-5 flex flex-col items-center justify-between rounded-md bg-neutral-200 "}"><h1 class="${"text-2xl font-semibold"}">₹39 Lac</h1>
                <button class="${"bg-indigo-600 mt-3 p-2 rounded-lg border-2 text-white border-indigo-600 font-semibold hover:text-indigo-600 hover:bg-white"}">Contact Dealer</button></div></div></div>
    ...
    <div class="${"relative"}"><div style="${"scroll-behavior: smooth;"}" class="${"mt-8 overflow-scroll "}"${add_attribute("this", scrollerMenu[0], 0)}><button class="${"absolute z-20 top-1/2 rounded-md bg-white -right-5"}"><img class="${"h-10"}" src="${"/RightArrow.svg"}" alt="${""}"></button>
            ...
            <div class="${"flex w-fit gap-5"}">${each([1, 2, 3, 4], (id) => {
    return `<img class="${"image h-[300px]"}"${add_attribute("src", `/images${id}.jpg`, 0)} alt="${""}">
                    <img class="${"image h-[300px]"}"${add_attribute("src", `/images${id}.jpg`, 0)} alt="${""}">`;
  })}</div></div></div>
</div>`;
});
export {
